...
const Sberspasibo = ({ min = 0, available = 1000, onChange }) => {
  ...
  const percentage =
    available > min ? ((selectedAmount - min) / (available - min)) * 100 : 0;

  return (
    <div className={styles.sberspasiboContainer}>
      <div className={styles.sberspasiboHeader}>
        СберСпасибо
      ...
      <div className={styles.sliderContainer}>
        ...
        />
        <div className={styles.selectedValue}>Списать: {selectedAmount} ₽</div>
      </div>
    </div>
  );
};
