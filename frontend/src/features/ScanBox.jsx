...
function ScanBox() {
  ...
  const handleSendImage = async (file, source) => {
    ...
    try {
      ...
      setResultMessage("Сканируем ваш чек..."); // Сообщение об отправке
      ...
      if (backendResponse?.receiptData?.items) {
        ...
        setResultMessage(
          `Чек успешно обработан! ID: ${backendResponse?.id || "N/A"}`
        );
      }
    } catch (err) {
  ...
  return (
    <div className={styles.scanBox}>
      {isLoading && (
        <p className={styles.statusMessage}>{resultMessage || "Загрузка..."}</p>
      )}
      {!isLoading && error && (
      ...
      {!isLoading && !error && resultMessage && (
        <p className={`${styles.statusMessage} ${styles.successMessage}`}>
          {resultMessage}
        </p>
      )}
      {!isLoading && (
        <div disabled={isLoading} className={styles.Welcome}>
          Добро пожаловать
        </div>
      )}
      {!isLoading && (
        <div disabled={isLoading} className={styles.WelcomeText}>
          Отсканируйте чек, чтобы разделить его между друзьями
        </div>
      )}
      {isLoading && (
        <div className={styles.roundLoadingContainer}>
          <img src={sberCat} alt="Sber Cat" className={styles.roundSpinner} />
        </div>
      )}
